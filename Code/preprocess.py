    ...
    def get_data(self, path, is_vgg, shuffle, augment):
        ...
        if augment:
            ...
            data_gen = tf.keras.preprocessing.image.ImageDataGenerator(
                preprocessing_function=self.preprocess_fn, rotation_range=10, shear_range=5,
        ...
        if not bool(self.idx_to_class):
            ...
            print("Unordered Length", len(unordered_classes))

            for img_class in unordered_classes:
                self.idx_to_class[data_gen.class_indices[img_class]] = img_class
                self.class_to_idx[img_class] = int(data_gen.class_indices[img_class])
                # print(len(self.classes))
                # print(int(data_gen.class_indices[img_class]))
                # print(len(img_class))
